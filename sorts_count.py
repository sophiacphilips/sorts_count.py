...
def insertion_count(a_list):
  ...
  for index in range(1, len(a_list)):
    ...
    comparisons += 1 #checking for initial comparisons in list
    while pos >= 0 and a_list[pos] > value:
      exchanges += 1 #checking for exchanges in the list
      a_list[pos + 1] = a_list[pos]
      pos -= 1
      comparisons += 1 #additional comparison, because if first comparison is false, second comparison won't be made
    a_list[pos + 1] = value
